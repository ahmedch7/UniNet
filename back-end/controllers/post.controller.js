...
export const reportPost = async (req, res) => {
  ...
  try {
    ...
    if (post.reportCount >= 5) {
      ...
      return res
        .status(200)
        .json({ message: "Post deleted due to multiple reports" });
    } else {
      await post.save();
...
export const verifyPost = async (req, res) => {
  ...
  try {
    ...
    post.isVerified = !post.isVerified; // Assuming 'isVerified' is a boolean field

    // Save the updated post
    await post.save();

    return res.json({ message: "Post verified successfully" });
  } catch (error) {
    console.error(error);
