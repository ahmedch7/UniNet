...
export async function getAvailableSalles(req, res) {
  const { date, heureDebut, heureFin } = req.body;

  try {
    // Convert date, heureDebut, and heureFin to Date objects
    const examDate = new Date(date);
    const startDateTime = new Date(`${date}T${heureDebut}:00.000Z`);
    const endDateTime = new Date(`${date}T${heureFin}:00.000Z`);

    // Find all salles
    const allSalles = await Salle.find();

    // Filter out salles that have conflicting schedules
    const availableSalles = allSalles.filter(salle => {
      return salle.schedules.every(schedule => {
        const scheduleStart = new Date(schedule.scheduledStartTime);
        const scheduleEnd = new Date(schedule.scheduledEndTime);

        return !(
          (scheduleStart <= startDateTime && scheduleEnd >= startDateTime) ||
          (scheduleStart <= endDateTime && scheduleEnd >= endDateTime) ||
          (scheduleStart >= startDateTime && scheduleEnd <= endDateTime)
        );
      });
    });
    ...
    res.status(500).json({ error: 'An error occurred while fetching available salles' });
  }
}